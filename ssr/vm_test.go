package ssr

import (
	"os"
	"sync"
	"testing"

	"github.com/dnsoa/go/assert"
)

var (
	ssrRenderPath = "ssr-render.js"
)

func TestVMRender(t *testing.T) {
	if os.Getenv("ssr_render_path") != "" {
		ssrRenderPath = os.Getenv("ssr_render_path")
	}
	r := assert.New(t)
	vm, err := NewVM(WithBundlerFile(ssrRenderPath))
	r.NoError(err)
	defer vm.Close()
	t.Run("RenderTemplate", func(t *testing.T) {
		r := assert.New(t)
		result, err := vm.RenderTemplate(`<div>{{ msg }}</div>`, map[string]any{"msg": "hello"})
		r.NoError(err)
		r.Equal(`<div>hello</div>`, result)
	})
	t.Run("RenderComponent", func(t *testing.T) {
		r := assert.New(t)
		result, err := vm.RenderComponent(`Tiny`, map[string]any{"msg": "hello"})
		r.NoError(err)
		r.Equal(`<div>hello</div>`, result)
	})
	t.Run("RenderPage", func(t *testing.T) {
		r := assert.New(t)
		result, err := vm.RenderPage(`tiny`, map[string]any{"title": "My Page", "msg": "hello"})
		r.NoError(err)
		r.Equal(`<div>Hello, hello.</div>`, result)
	})
}

func TestStyle(t *testing.T) {
	r := assert.New(t)
	vm, err := NewVM(WithBundlerFile(ssrRenderPath))
	r.NoError(err)
	defer vm.Close()
	result, err := vm.RenderPage(`index/index`, map[string]any{
		"text": "My Page",
		"posts": []map[string]any{
			{
				"id":   6076008594505252933,
				"name": "RMHK",
				"body": "SpmvIpm",
			},
			{
				"id":   2112069284341772391,
				"name": "hRKc",
				"body": "GoJglxJiGo",
			},
			{
				"id":   8410961337293083680,
				"name": "bBfi",
				"body": "INnVnlYO",
			},
			{
				"id":   2568248320646248211,
				"name": "gmEwO",
				"body": "iaafuXcq",
			},
			{
				"id":   5723807497442001082,
				"name": "nQTEmObp",
				"body": "NyQDKEUl",
			},
		},
	})
	r.NoError(err)
	r.Equal(`<div class="red">Hello, hello.</div>`, result)
}
func BenchmarkVMRender(b *testing.B) {
	vm, err := NewVM(WithBundlerFile(ssrRenderPath))
	if err != nil {
		b.Fatal(err)
	}
	defer vm.Close()
	b.Run("RenderTemplate", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := vm.RenderTemplate(`<div>{{ msg }}</div>`, map[string]any{"msg": "hello"})
			if err != nil {
				b.Fatal(err)
			}
		}
	})
	b.Run("RenderComponent", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := vm.RenderComponent(`Tiny`, map[string]any{"msg": "hello"})
			if err != nil {
				b.Fatal(err)
			}
		}
	})
}

func TestVMRender_Concurrent(t *testing.T) {
	r := assert.New(t)
	vm, err := NewVM(WithBundlerFile(ssrRenderPath))
	r.NoError(err)
	defer vm.Close()
	ch := make(chan struct{})
	n := 10
	for i := 0; i < n; i++ {
		go func() {
			<-ch
			result, err := vm.RenderTemplate(`<div>{{ msg }}</div>`, map[string]any{"msg": "hello"})
			r.NoError(err)
			r.Equal(`<div>hello</div>`, result)
		}()
	}
	close(ch)
}

func BenchmarkVMRender_Concurrent(b *testing.B) {
	vm, err := NewVM(WithBundlerFile(ssrRenderPath))
	if err != nil {
		b.Fatal(err)
	}
	defer vm.Close()
	ch := make(chan struct{})
	n := 10

	var wg sync.WaitGroup
	wg.Add(n)
	errs := make(chan error, n)
	for i := 0; i < n; i++ {
		go func() {
			defer wg.Done()
			<-ch
			for j := 0; j < b.N; j++ {
				result, err := vm.RenderTemplate(`<div>{{ msg }}</div>`, map[string]any{"msg": "hello"})
				if err != nil {
					errs <- err
					return
				}
				if result != `<div>hello</div>` {
					errs <- err
					return
				}
			}
		}()
	}
	close(ch)
	wg.Wait()
	close(errs)
	for err := range errs {
		if err != nil {
			b.Fatal(err)
		}
	}
}
