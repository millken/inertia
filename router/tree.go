...
func (tree *Tree[T]) Lookup(path string) (bool, T, []Parameter) {
	var params []Parameter

	found, data := tree.LookupNoAlloc(path, func(key string, value string) {
		params = append(params, Parameter{key, value})
	})

	return found, data, params
}

// LookupNoAlloc finds the data for the given path without using any memory allocations.
func (tree *Tree[T]) LookupNoAlloc(path string, addParameter func(key string, value string)) (bool, T) {
	var (
		i             uint
	...
	for i < uint(len(path)) {
		...
		if i == uint(len(node.prefix)) {
			...
			if node.wildcard != nil {
				...
				return true, node.wildcard.data
			}
...
notFound:
	if parameter != nil {
		...
		return true, parameter.data
	}

	if wildcard != nil {
		addParameter(wildcard.prefix, wildcardPath)
		return true, wildcard.data
	}
