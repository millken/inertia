...
func New() *Builder {
	return &Builder{http.DefaultTransport}
}
...
func SSR(flag *Flag) esbuild.BuildOptions {
	options := esbuild.BuildOptions{
		...
		GlobalName: "",
		Conditions: []string{"svelte", "default", "import", "production"},
		// Always bundle, use plugins to granularly mark files as external
		Bundle:       true,
		MangleQuoted: esbuild.MangleQuotedTrue,
		MangleProps:  "_$",
...
func resolveNodeModule(fsys fs.FS, module string) (string, error) {
	...
	// if (hasScope && len(sections) > 1) || (!hasScope && len(sections) > 1) {
...
func notFoundPlugin() esbuild.Plugin {
	return esbuild.Plugin{
		Name: "not_found",
		Setup: func(build esbuild.PluginBuild) {
